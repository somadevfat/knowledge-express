...
全ページ共通フッターにサイト全体のアクセス数を表示する。Cloudflare D1（`site_views`テーブル、1行のみ）にDrizzle ORM経由でupsertする（`src/features/knowledge/server/view-count.ts`）。記事ごとの閲覧数は持たない。D1バインディングが無いランタイムでは機能を無効化し、フッター自体を表示しない。
