...
export class Knowledge {
  ...
  /**
   * 永続化済みデータやテストデータからEntityを復元する。
   *
   * @param props 復元するナレッジ記事の値。
   * @returns 復元したナレッジEntity。
   * @throws {DomainError} 記事として必要な値が不足している場合。
   */
  static reconstruct(props: KnowledgeProps): Knowledge {
    return new Knowledge({
      id: validateId(props.id),
      title: validateTitle(props.title),
      body: validateBody(props.body),
      excerpt: validateExcerpt(props.excerpt),
      tags: normalizeTags(props.tags),
      category: validateCategory(props.category),
      path: validatePath(props.path),
      order: props.order,
      sourceUrl: validateSourceUrl(props.sourceUrl),
      updatedAt: props.updatedAt === undefined ? undefined : new Date(props.updatedAt)
    });
  }
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (key === "tags" && rawValue === "") {
      ...
      for (let tagIndex = index + 1; tagIndex < lines.length; tagIndex += 1) {
        ...
        tags.push(stripQuotes(tagLine.slice(2).trim()));
        index = tagIndex;
      }
      result.tags = tags;
      continue;
    ...
    if (key === "id") result.id = rawValue;
    if (key === "title") result.title = rawValue;
    if (key === "excerpt") result.excerpt = rawValue;
    if (key === "category") result.category = rawValue;
    if (key === "order") result.order = Number(rawValue);
    if (key === "tags" && rawValue !== "") result.tags = rawValue.split(",").map((tag) => tag.trim());
  }
